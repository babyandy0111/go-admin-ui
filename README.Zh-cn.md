...
基于Gin + Vue + Element UI的前後端分离權限管理系统,系统初始化极度简单，只需要設定文件中，修改資料库连接，系统支持多指令操作，迁移指令可以让初始化資料库訊息变得更简单，服务指令可以很简单的啟动api服务
...
- 基于 GORM 的資料库存储，可扩展多种类型資料库 

- 設定文件简单的模型映射，快速能够得到想要的設定
...
1. 用户管理：用户是系统操作者，该功能主要完成系统用户設定。
2. 部門管理：設定系统组织机构（公司、部門、小组），树结构展现支持資料權限。
3. 職位管理：設定系统用户所属担任职务。
4. 選單管理：設定系统選單，操作權限，按钮權限标识，接口權限等。
5. 角色管理：角色選單權限分配、设置角色按机构进行資料範圍權限划分。
6. 字典管理：对系统中经常使用的一些较為固定的資料进行维护。
7. 参数管理：对系统动态設定常用参数。
8. 操作日志：系统正常操作日志记录和查询；系统异常訊息日志记录和查询。
9. 登录日志：系统登录日志记录查询包含登录异常。
1. 接口文档：根据业务代码自动生成相关的api接口文档。
1. 代码生成：根据資料表结构生成对应的增删改查相对应业务，全程可视化操作，让基本业务可以零代码实现。
1. 表单构建：自定义頁面样式，拖拉拽实现頁面布局。
1. 服务监控：查看一些服务器的基本訊息。
